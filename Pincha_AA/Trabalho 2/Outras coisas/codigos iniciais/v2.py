...
def exaustive(s,K,G):
    ...
    conf_max = min(n_comb,10)

    while (N_conf_E<=conf_max):
        N_conf_E = N_conf_E + 1

        idx = random.randint(0,n_comb-1)

        while(idx in indxs):
            idx = random.randint(1,n_comb-1)
...
def main():
    ...
    S = [x for x in range(10,21)]                                            # Tamanho do grafo
    ...
    for z in range(len(P_list)):
        ...
        for w in range(len(S)):
            ...
            while(Total_Tested <= 5):

                K = random.randint(2,s-1)
                ...
                if len(S_E) != 0:
                    ...
                    if op_exaustive < op_best:                          # compara o nº de operações com a melhor até ao momento
                        ...
                        k_vector[w] = K
                        n_solutions_E[w] = n_solutions_E[w] + 1
                        solutions[w] = set(S_E)
